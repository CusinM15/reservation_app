...
- **Cloudflare** proxies to LoadBalancer IP `{{LB_IP}}` (MetalLB, port 80) or alternatively to any node via nginx (port {{NGINX_PORT}})
- **Nginx** on **k3s-1** and **k3s-2** (both identical) proxy-passes to `{{LB_IP}}:{{LB_PORT}}`
